...
var SignalRService = function () {
    ...
    var SignalRService = _classThis = /** @class */ (function () {
        function SignalRService_1(host) {
            this.host = host;
        }
        SignalRService_1.prototype.startConnection = function () {
            var _this = this;
            ...
            this.hubConnection
                ...
                _this.enviarMensaje("Prueba");
            })
                .catch(function (err) { return console.error('Error al conectar con SignalR:', err); });
        };
        SignalRService_1.prototype.enviarMensaje = function (mensaje) {
            this.hubConnection.invoke('Send', mensaje) // Reemplaza 'EnviarMensaje' con el nombre del método en tu servidor SignalR
                .catch(function (err) { return console.error('Error al enviar mensaje a SignalR:', err); });
        };
        // Método para escuchar eventos desde el servidor
        SignalRService_1.prototype.addReceiveMessageListener = function (callback) {
            this.hubConnection.on('msg', function (message) {
                callback(message);
            });
        };
